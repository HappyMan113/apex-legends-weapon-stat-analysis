...
_SIDEARM: RequiredTerm = Term('sidearm', 'sodarm', 'sadarm', 'sad arm') | (_SIDE + _ARM)
_SIDEARMS: RequiredTerm = Term('sidearms', 'sodarms', 'sadarms', 'sad arms') | (_SIDE + _ARMS)
...
SIDEARM_OPT_OF = _SIDEARM + Term('of').opt()
WITH_SIDEARM = AND | (AND | _SIDEARM) | ((WITH | SWITCHING_TO) + SIDEARM_OPT_OF) | SIDEARM_OPT_OF
MAIN = Term('main', 'may', 'name', 'primary')
_THE_SAME = Term('the').opt() + Term('same')
THE_SAME_SIDEARM: RequiredTerm = _THE_SAME + _SIDEARM
...
CARE_PACKAGE: RequiredTerm = Term('care package', 'supply drop')
CARE_PACKAGE_OPT: OptTerm = CARE_PACKAGE.opt()
ALTERNATOR = Term('Alternator', 'I don\'t need her', 'I\'ll do neither')
_CHARGE = Term('Charge', 'charger', 'charged', 'ciao')
_RIFLE = Term('Rifle', 'rifles', 'full', 'dreffel')
CHARGE_RIFLE = (_CHARGE + _RIFLE) | Term('charger')
